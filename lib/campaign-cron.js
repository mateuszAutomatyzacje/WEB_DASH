...
import {
  campaignStatusAllowsAutoSend,
  campaignStatusAllowsAutoSync,
  getCampaignRuntimeState,
  getNextAllowedEmailSendAt,
  getNextExpectedAt,
  getSendWindowStatus,
  resolveOrCreateCampaign,
  runCampaignGuard,
} from '@/lib/campaign-guard.js';
...
function resolveIntervalOverride(value, fallback) {
...
function getTaskDueAtMs({ nextExpectedAt = null, lastAt = null, intervalMin = 10, nowMs = Date.now() }) {
  const nextExpectedMs = parseDateMs(nextExpectedAt);
  if (nextExpectedMs !== null) return nextExpectedMs;

  const lastMs = parseDateMs(lastAt);
  if (lastMs === null) return nowMs;

  return lastMs + toPositiveInt(intervalMin, 10, { min: 1, max: 1440 }) * 60 * 1000;
}

function getNextExpectedEmailAt(baseMs) {
  const safeBaseMs = Number.isFinite(baseMs) ? baseMs : Date.now();
  return getNextAllowedEmailSendAt(new Date(safeBaseMs)) || new Date(safeBaseMs).toISOString();
}
...
export function shouldRunAutomaticCampaignTick(campaign, nowMs = Date.now()) {
  ...
  const syncAllowedByCampaignStatus = campaignStatusAllowsAutoSync(campaign.status);
  const sendAllowedByCampaignStatus = campaignStatusAllowsAutoSend(campaign.status);

  const syncDue = isTaskDue({
    enabled: syncAllowedByCampaignStatus && runtime.auto_sync_enabled,
  ...
  const sendDue = isTaskDue({
    enabled: sendAllowedByCampaignStatus && runtime.auto_send_enabled,
    nextExpectedAt: runtime.next_expected_send_at,
    lastAt: runtime.last_auto_send_at,
    intervalMin: runtime.send_email_interval_min,
    nowMs,
  });
  const sendWindowOpen = !getSendWindowStatus(new Date(nowMs)).blocked;

  return syncDue || (sendDue && sendWindowOpen);
}
...
export async function runCampaignCronTick(sql, {
  ...
  const lockResult = await withCampaignLock(sql, campaign.id, async (lockedSql) => {
    ...
    const runtime = getCampaignRuntimeState(settings);
    const syncAllowedByCampaignStatus = campaignStatusAllowsAutoSync(freshCampaign.status);
    const sendAllowedByCampaignStatus = campaignStatusAllowsAutoSend(freshCampaign.status);
    const configuredAutoSyncEnabled = forceSync === true ? true : runtime.auto_sync_enabled;
    const configuredAutoSendEnabled = forceSend === true ? true : runtime.auto_send_enabled;
    const effectiveAutoSyncEnabled = forceSync === true ? true : (configuredAutoSyncEnabled && syncAllowedByCampaignStatus);
    const effectiveAutoSendEnabled = forceSend === true ? true : (configuredAutoSendEnabled && sendAllowedByCampaignStatus);
    const storedLeadSyncIntervalMin = toPositiveInt(
      Number(runtime.lead_sync_interval_min || settings.lead_sync_interval_min || settings.send_interval_min || 30),
      30,
      { min: 1, max: 1440 },
    );
    const storedSendEmailIntervalMin = toPositiveInt(
      Number(runtime.send_email_interval_min || settings.send_email_interval_min || 5),
      5,
      { min: 1, max: 1440 },
    );
    const resolvedLeadSyncIntervalMin = toPositiveInt(
      resolveIntervalOverride(leadSyncIntervalMin ?? intervalMin, storedLeadSyncIntervalMin),
      storedLeadSyncIntervalMin,
      { min: 1, max: 1440 },
    );
    const resolvedSendEmailIntervalMin = toPositiveInt(
      resolveIntervalOverride(sendEmailIntervalMin, storedSendEmailIntervalMin),
      storedSendEmailIntervalMin,
      { min: 1, max: 1440 },
    );
    const resolvedLimit = resolveSendLimit(limit, settings);
    const nowMs = Date.now();
    const shouldSync = forceSync === true || isTaskDue({
      enabled: effectiveAutoSyncEnabled,
      nextExpectedAt: runtime.next_expected_sync_at,
      lastAt: runtime.last_sync_at,
      intervalMin: resolvedLeadSyncIntervalMin,
      nowMs,
    });
    const sendDueAtMs = getTaskDueAtMs({
      enabled: effectiveAutoSendEnabled,
      nextExpectedAt: runtime.next_expected_send_at,
      lastAt: runtime.last_auto_send_at,
    ...
    const sendWindowStatus = getSendWindowStatus(new Date(Math.max(sendDueAtMs, nowMs)));
    const sendBlockedByWindow = effectiveAutoSendEnabled && forceSend !== true && sendDue && sendWindowStatus.blocked;
    const shouldSend = forceSend === true || (sendDue && !sendBlockedByWindow);

    const nextExpectedSyncAt = shouldSync
      ? getNextExpectedAt(resolvedLeadSyncIntervalMin)
      : (runtime.next_expected_sync_at || null);
    const nextExpectedSendAt = effectiveAutoSendEnabled
      ? getNextExpectedEmailAt(
        shouldSend
          ? nowMs + resolvedSendEmailIntervalMin * 60 * 1000
          : Math.max(sendDueAtMs, nowMs),
      )
    ...
    await logApplicationEventSafe(lockedSql, {
      ...
      details: {
        ...
        limit: resolvedLimit,
        sync_allowed_by_campaign_status: syncAllowedByCampaignStatus,
        send_allowed_by_campaign_status: sendAllowedByCampaignStatus,
        send_blocked_by_window: sendBlockedByWindow,
        send_window: sendWindowStatus,
      },
    });
    ...
    if (!shouldSync && !shouldSend && !dryRun) {
      ...
      return {
        ...
        auto_sync_enabled: configuredAutoSyncEnabled,
        auto_send_enabled: configuredAutoSendEnabled,
        next_expected_sync_at: nextExpectedSyncAt,
        next_expected_send_at: nextExpectedSendAt,
        queued: 0,
        sent: 0,
    ...
    const mergedSettings = {
      ...settings,
      auto_sync_enabled: configuredAutoSyncEnabled,
      auto_sync_status: configuredAutoSyncEnabled ? 'running' : 'paused',
      auto_send_enabled: configuredAutoSendEnabled,
      auto_send_status: configuredAutoSendEnabled ? 'running' : 'paused',
      lead_sync_interval_min: resolvedLeadSyncIntervalMin,
      send_email_interval_min: resolvedSendEmailIntervalMin,
