...
export function shouldRunAutomaticCampaignTick(campaign, nowMs = Date.now()) {
  ...
  const intervalMs = toPositiveInt(runtime.sync_interval_min, 10, { min: 1, max: 1440 }) * 60 * 1000;
...
export async function runCampaignCronTick(sql, {
  ...
  const lockResult = await withCampaignLock(sql, campaign.id, async (lockedSql) => {
    ...
    const resolvedIntervalMin = toPositiveInt(
      intervalMin,
      Number(runtime.sync_interval_min || settings.send_interval_min || 10),
      { min: 1, max: 1440 },
    );
    ...
    await lockedSql`
      ...
      set
        ...
        || jsonb_build_object(
          'last_sync_result', ${lockedSql.json(sendResult?.sync || {})}::jsonb,
          'last_send_result', ${lockedSql.json(schedulerResult)}::jsonb,
          'last_scheduler_result', ${lockedSql.json(schedulerResult)}::jsonb
        ),
        updated_at = now()
