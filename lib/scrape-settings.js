...
async function loadRunnerCampaign(sql, campaignName) {
  let rows = [];
...
export async function getScrapeIntervalMin(sql, { campaignName = null } = {}) {
  const campaign = await loadRunnerCampaign(sql, campaignName);
  if (!campaign) return { intervalMin: null, source: null, campaign: null };

  const settings = normalizeStoredCampaignSettings(campaign.settings);
  const raw = typeof settings?.sync_interval_min !== 'undefined'
    ? settings?.sync_interval_min
    : settings?.send_interval_min;

  const intervalMin = toIntervalMin(raw, { min: 1, max: 1440 });
  const source = typeof settings?.sync_interval_min !== 'undefined' ? 'sync_interval_min' : 'send_interval_min';
