...
export const SEND_INTERVAL_OPTIONS = [5, 10, 15, 30];
...
export function normalizeStoredCampaignSettings(settings = {}) {
  ...
  const sendIntervalMin = toIntervalMin(
    typeof resolved?.send_interval_min !== 'undefined'
      ? resolved?.send_interval_min
      : resolved?.sync_interval_min,
  );
  if (sendIntervalMin !== null) normalized.send_interval_min = sendIntervalMin;
