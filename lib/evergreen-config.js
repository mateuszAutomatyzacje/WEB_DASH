...
export function getCampaignRunnerConfig(source = {}) {
  const settings = isPlainObject(source?.settings) ? source.settings : (isPlainObject(source) ? source : {});
...
export function normalizeStoredCampaignSettings(settings = {}) {
  ...
  const parsedRunner = parseJsonObject(settings?.evergreen_runner);
  if (parsedRunner) normalized.evergreen_runner = parsedRunner;
  return normalized;
}

export function buildEditableCampaignSettings(settings = {}) {
  const cleanSettings = isPlainObject(settings) ? { ...settings } : {};
