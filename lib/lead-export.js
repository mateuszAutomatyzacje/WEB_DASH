...
function getLeadExportHeaderCellRange(columnCount, rowIndex) {
...
export function buildLeadExportWorkbook(rows = [], generatedAt = new Date().toISOString()) {
  const mapped = rows.map(mapLeadExportRow);
  const headers = getLeadExportHeaders();
  ...
  const sheetRows = [
    ...
    [],
    headers,
    ...mapped.map((row) => headers.map((header) => row[header] ?? '')),
  ];

  const worksheet = XLSX.utils.aoa_to_sheet(sheetRows);
  const headerRange = getLeadExportHeaderCellRange(headers.length, headerRowIndex);
  if (headerRange) {
  ...
  worksheet['!cols'] = headers.map((header) => ({
    wch: Math.min(
      ...
      Math.max(
        header.length + 2,
        ...mapped.map((row) => String(row[header] ?? '').slice(0, 60).length + 2),
      ),
    ),
  }));

  const hyperlinkHeaders = new Set(['job_url', 'linkedin_url', 'website_url']);
  for (let rowIndex = 0; rowIndex < mapped.length; rowIndex += 1) {
    for (let columnIndex = 0; columnIndex < headers.length; columnIndex += 1) {
      const header = headers[columnIndex];
      if (!hyperlinkHeaders.has(header)) continue;
      const value = String(mapped[rowIndex]?.[header] ?? '').trim();
      if (!value) continue;
      const cellRef = XLSX.utils.encode_cell({ r: dataRowStartIndex + rowIndex, c: columnIndex });
      if (!worksheet[cellRef]) continue;
      worksheet[cellRef].l = { Target: value, Tooltip: value };
    }
  }
...
export function buildLeadExportXlsxBuffer(rows = [], generatedAt = new Date().toISOString()) {
  const workbook = buildLeadExportWorkbook(rows, generatedAt);
