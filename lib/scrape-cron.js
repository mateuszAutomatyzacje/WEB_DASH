...
const DEFAULT_SCRAPE_WEEKEND_INTERVAL_MIN = 120;
const DEFAULT_SCRAPE_WEEKEND_STOP = '18:00:00';
const DEFAULT_SCRAPE_WEEKEND_RESUME = '04:00:00';
...
function getScrapeTimeZone() {
  return String(
    process.env.WEBDASH_SCRAPE_TIMEZONE
      || process.env.WEBDASH_SEND_TIMEZONE
      || DEFAULT_LOCAL_TIMEZONE,
  ).trim() || DEFAULT_LOCAL_TIMEZONE;
}

function getScrapeWeekendCadence(date = new Date(), baseIntervalMin = 30) {
  const timeZone = getScrapeTimeZone();
  const quietStart = parseTimeValue(process.env.WEBDASH_SCRAPE_WEEKEND_STOP_LOCAL || DEFAULT_SCRAPE_WEEKEND_STOP, DEFAULT_SCRAPE_WEEKEND_STOP);
  const quietResume = parseTimeValue(process.env.WEBDASH_SCRAPE_WEEKEND_RESUME_LOCAL || DEFAULT_SCRAPE_WEEKEND_RESUME, DEFAULT_SCRAPE_WEEKEND_RESUME);
  const weekendIntervalMin = toPositiveInt(
    process.env.WEBDASH_SCRAPE_WEEKEND_INTERVAL_MIN,
    DEFAULT_SCRAPE_WEEKEND_INTERVAL_MIN,
    { min: 1, max: 1440 },
  );
  const localTime = getLocalDateTimeParts(date, timeZone);
  const currentSeconds = toLocalSeconds(localTime);
  const quietStartSeconds = toLocalSeconds(quietStart);
  const quietResumeSeconds = toLocalSeconds(quietResume);
  const weekendDay = isWeekendWeekdayIndex(localTime.weekday_index);
  const carryFromSundayNight = localTime.weekday_index === 1 && currentSeconds < quietResumeSeconds;
  const quietBlocked = (weekendDay || carryFromSundayNight) && isTimeInWindow(currentSeconds, quietStartSeconds, quietResumeSeconds);
  ...
  return {
    ...
    quiet_blocked: quietBlocked,
    interval_min: weekendDay ? weekendIntervalMin : baseIntervalMin,
    base_interval_min: baseIntervalMin,
    weekend_interval_min: weekendIntervalMin,
    quiet_start_at: quietStart.label,
    quiet_resume_at: quietResume.label,
  };
}

function findNextScrapeEligibleAt(nowMs, baseIntervalMin) {
  let probeMs = nowMs;
  for (let i = 0; i < 48 * 60; i += 1) {
    const status = getScrapeWeekendCadence(new Date(probeMs), baseIntervalMin);
    if (!status.quiet_blocked) {
      return {
        nextEligibleAt: new Date(probeMs).toISOString(),
        cadence: status,
      };
    }
    probeMs += 60 * 1000;
  ...
  return {
    nextEligibleAt: null,
    cadence: getScrapeWeekendCadence(new Date(nowMs), baseIntervalMin),
  };
}
...
export async function getScrapeTickState(sql, { campaignId = null, campaignName = null, nowMs = Date.now() } = {}) {
  ...
  const cadence = getScrapeWeekendCadence(new Date(nowMs), schedule.intervalMin);

  const lastRunMs = parseDateMs(cfg.last_run_at);
  const nextRunMs = lastRunMs === null ? null : lastRunMs + cadence.interval_min * 60 * 1000;

  if (cadence.quiet_blocked) {
    const nextEligible = findNextScrapeEligibleAt(nowMs, schedule.intervalMin);
    const nextEligibleMs = parseDateMs(nextEligible.nextEligibleAt);
    const deferredRunMs = nextRunMs === null
      ? nextEligibleMs
      : (nextEligibleMs === null ? nextRunMs : Math.max(nextRunMs, nextEligibleMs));

    return {
      ok: true,
      skipped: true,
      reason: 'weekend_quiet_hours',
      settings: cfg,
      interval_min: cadence.interval_min,
      interval_source: cadence.weekend_day ? 'weekend_override' : schedule.source,
      campaign: schedule.campaign,
      next_run_at: deferredRunMs === null ? null : new Date(deferredRunMs).toISOString(),
      time_zone: cadence.time_zone,
      local_time: cadence.local_time,
      local_weekday: cadence.local_weekday,
    };
  }
