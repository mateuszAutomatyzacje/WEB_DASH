...
const AUTO_SEND_ALLOWED_CAMPAIGN_STATUSES = new Set(['running', 'paused', 'stopped']);
...
export function campaignStatusAllowsAutoSync(status) {
  return String(status || '').trim().toLowerCase() === 'running';
}

export function campaignStatusAllowsAutoSend(status) {
  return AUTO_SEND_ALLOWED_CAMPAIGN_STATUSES.has(String(status || '').trim().toLowerCase());
}
