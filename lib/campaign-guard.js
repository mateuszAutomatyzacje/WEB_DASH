...
function parseTimeValue(value, fallbackLabel) {
...
export function getSendWindowStatus(date = new Date()) {
  ...
  return {
    blocked: beforeStart || afterCutoff,
...
export function getCampaignRuntimeState(settings = {}) {
  ...
  return {
    ...
    next_expected_send_at: deriveNextExpectedAt(normalized.next_expected_send_at || null, lastAutoSendAt, config.sendEmailIntervalMin),
