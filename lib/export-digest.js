...
const DEFAULT_BINARY_FIELD_NAME = 'data';
...
function getDigestConfig() {
  return {
    ...
    binaryFieldName: normalizeText(process.env.WEBDASH_EXPORT_DIGEST_BINARY_FIELD_NAME, DEFAULT_BINARY_FIELD_NAME),
  };
}
...
function buildAttachmentFilename() {
  return `lead-export-${new Date().toISOString().slice(0, 10)}.xlsx`;
}
...
function buildAttachmentPayload(rows, generatedAt) {
  ...
  const buffer = buildLeadExportXlsxBuffer(rows, generatedAt);
...
async function triggerExportDigestWebhook(payload, { webhookUrl, token, attachment, binaryFieldName = DEFAULT_BINARY_FIELD_NAME }) {
  ...
  if (attachment?.buffer) {
    const fileBlob = new Blob([attachment.buffer], { type: attachment.contentType });
    form.append(binaryFieldName, fileBlob, attachment.filename);
  }
  ...
  const lockResult = await withDigestLock(sql, config.recipientEmail, async (lockedSql) => {
    ...
    const runRows = await lockedSql`
      ...
      values (
        ...
        ${filters.email},
        ${filters.updated},
        ${rowsForSend.length > 0 ? 'running' : 'no_new_records'},
        ${rowsForSend.length},
        ${config.webhookUrl},
        ${lockedSql.json({
          ...
          pending_row_count: pendingRows.length,
          test_fallback_rows: test && pendingRows.length === 0 ? rowsForSend.length : 0,
        })}::jsonb,
        now()
      )
      returning id
    ...
    try {
      if (rowsForSend.length === 0) {
        ...
        return {
          ok: true,
          skipped: true,
          reason: test ? 'no_test_rows' : 'no_new_records',
          run_id: runId,
          pending_count: 0,
        };
      }
      ...
      const webhook = await triggerExportDigestWebhook({
        ...
        headers_json: getLeadExportHeaders(),
        ...
        attachment,
        binaryFieldName: config.binaryFieldName || DEFAULT_BINARY_FIELD_NAME,
      });
