...
export default function EmailSendingControlPanel({ campaignName, campaignId, initial }) {
  ...
  async function run(action) {
    ...
    try {
      ...
      if (action === 'test') setMsg(`TEST OK: queued=${data?.queued ?? 0}`);
  ...
  return (
    <section style={{ border: '1px solid #1f2937', borderRadius: 16, padding: 16, background: '#000', color: '#f8fafc' }}>
      ...
      <div style={{ fontSize: 13, color: '#cbd5e1', lineHeight: 1.6, marginBottom: 12 }}>
        Scheduler <code>POST /api/admin/campaign/cron-sync</code> robi automatyczna wysylke tylko gdy email sending jest wlaczony.
      </div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        ...
        <button onClick={() => run('test')} disabled={loading} style={{ background: '#111827', color: '#f8fafc', border: '1px solid #374151', borderRadius: 8, padding: '8px 10px' }}>{loading ? '...' : 'Test send (dry-run)'}</button>
